...
- **Cost decides the rule.** `components.m.cost` present (copper) → must possess the NAMED component (`getGoldComponentCandidates`, name-keyword match + value floor, stem-tolerant). No cost (plain string) → needs a focus/pouch/substitute (`getSpellcastingFocusStatus`). `consume` is independent of cost.
...
- **Require the named component; a valuable is not a substitute (R39 follow-up).** The first cut matched gold-cost components as a HYBRID (name keyword **or** value ≥ cost) and auto-consumed the best candidate on the default quick-cast — so a player with a random valuable (no named component) could cast *and* have that unrelated item silently destroyed ("it just consumes random items"). Fixed in two parts: (1) `getGoldComponentCandidates` matches by NAME (a merely-valuable item is not "the component") AND applies a value floor to items carrying an explicit price (a "Diamond" worth 200 gp fails a 300 gp cost; value-less homebrew items are trusted by name). The GATE blocks a gold-cost cast unless you own a qualifying named item; the method returns `[]` when the text names nothing. (2) `_pConsumeMaterialComponent` consumes a single named component directly and prompts (`_pPromptConsumeComponentChoice`) only when several named items qualify or the consume is `"optional"`; `skipComponentPrompt` never authorizes silent destruction. Net: a value-only item is never a candidate — never blocks-through, never prompted, never destroyed.

**Focus substitutions** (`getSpellcastingFocusStatus`): SCF item / `scfType`, component pouch (by name), Spellsword Technique + melee weapon, War Caster + shield, Star Map feature, Gambler's Spellcasting + cards/dice/coins. Possession (not equip-state) is sufficient to avoid false-blocks. Returns `{ok, source, itemName}` — `source` is a friendly kind derived from `scfType` ("arcane focus"/"druidic focus"/"holy symbol") or a shortened substitution label; `itemName` is the specific inventory item (null for feature-only substitutions like Star Map).

**Focus detail in the cast readout (R39 follow-up 2)**: `_getSpellFocusNote(spell, spellData, {variantUsed})` (spells) returns a short label for the `🔮 Focus:` toast line — only for a no-cost (`requiresFocus`) material spell where a focus is possessed and no variant was used. Label = `itemName (source)`, with source suppressed when the item name already contains it (e.g. "Component Pouch"), or just `source` for feature-only substitutions ("Star Map"). Wired into `_handleSpellEffects` after the variant-component line; NOT emitted from `_castInnateSpell`.

**Verification**: pinned by `CharacterSheetSpellComponents.test.js` (42 tests). (R39.)
