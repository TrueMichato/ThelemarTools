...
Some features let a skill be computed from a *different* ability than its default (e.g. Forest Sage: "use your choice of Intelligence or Wisdom to make Arcana, Nature, Animal Handling, or Survival checks"; TGTT Barbarian's Mark of the Wilderness). This is modeled generically with **named modifiers of type `abilitySwap:<normalizedSkill>`** carrying a `newAbility` (3-letter abv). `getSkillMod` and `getSkillBreakdown` take `MAX(default-ability mod, every currently eligible swap newAbility mod)` for that skill — player-favorable and stacking-safe when multiple features touch the same skill. Manual pins and per-roll overrides take precedence. When an always-on swap wins, the breakdown labels the ability `"<ABL> modifier (swapped from <DEFAULT>)"`.
...
`FeatureModifierParser` also handles "use Strength instead of Charisma for [multiple skills] checks" as a whole clause, including HTML-rendered tags: Mark of the Wilderness swaps **Intimidation and Persuasion**, while its separate proficiency-bonus grant applies **only to Intimidation**. The XPHB (and TGTT-referenced) level-3 Primal Knowledge has a different gate: its five Strength alternatives (Acrobatics, Intimidation, Perception, Stealth, Survival) carry `requiresStateTypeId: "rage"`. `_resolveSkillAbility` ignores them outside active Rage, then auto-selects the better ability during Rage. `getSkillBreakdown` treats only the *difference* from the usual ability as situational, leaving the canonical value intact. `_migrateAbilityChoiceSwaps` reparses saved feature descriptions, preserves the active-state requirement, and deduplicates source-linked swaps; it never turns a Rage-only swap into a permanent one.

For a skill roll with an explicit per-roll ability override, `_rollSkillCheck` passes that ability to `getAdvantageState("skill:<name>", {overrideAbility})`. This keeps skill-specific state effects while matching ability-check state effects (such as Rage's Strength-check advantage) against the ability actually rolled.
