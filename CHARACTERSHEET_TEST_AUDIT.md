...
| File | Tests | Coverage |
|---|---|---|
| CharacterSheetReanimatorR2cFacilitatedRevival.test.js | 26 | Exact `Artificer|EFA` / `Reanimator|RHW` level-15 Facilitated Revival execution; XPHB Artisan focus identity/proficiency/inventory gates; atomic feature-resource plus committed-cast receipt ordering; slot/material waiver; stable focus IDs; hook-failure semantics; byte-for-byte rejection paths; long-rest recovery; level-17/player spell collisions; export/load migration; teardown; Respec Apply/Undo/failure |
