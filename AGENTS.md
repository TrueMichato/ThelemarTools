...
`charactersheet.js`(6.5K)=controller/orchestrator | `charactersheet-state.js`(23.4K)=state/model | `charactersheet-builder.js`(6.5K)=creation wizard | `charactersheet-levelup.js`(4K)=single level-up | `charactersheet-quickbuild.js`(3K)=multi-level build | `charactersheet-combat.js`(3.9K)=attacks/conditions/death saves | `charactersheet-spells.js`(3.3K)=spell slots/casting | `charactersheet-inventory.js`(2.3K)=items/equipment | `charactersheet-features.js`(1.6K)=feature display | `charactersheet-rest.js`(630)=short/long rest | `charactersheet-class-utils.js`(1.8K)=static helpers | `charactersheet-spell-picker.js`(1.2K)=reusable spell UI | `charactersheet-buffpicker-helpers.js`(170)=Apply Buff modal helpers (pure, no DOM) | `charactersheet-customabilities.js`(800)=homebrew abilities | `charactersheet-npc-exporter.js`(1.5K)=NPC statblock export | `charactersheet-export.js`(320)=import/export | `charactersheet-respec.js`(600)=level history | `charactersheet-layout.js`(800)=drag-drop layout | `charactersheet-notes.js`(500)=sticky notes
...
- **Party Tracker** — 5 modules in `js/dmscreen/partytracker/` for managing party character stats, HP tracking, character sheet import, DC probabilities, and TGTT homebrew settings
- **Journey Tracker** — 1 module (`js/dmscreen/dmscreen-journeytracker.js`, ~3100 lines) for overland travel segments, activities, Risk Modifier, camp phases, weather system, supply tracking, and day timeline
...
- Weather system: 10 built-in types + custom types. Area weather tables with 6 presets. 🎲 Roll Weather button. Per-segment weather. DC/RM/pace modifiers.
- Supply tracker: auto-depletion on New Day, party-size sync, Extreme Heat doubles water.
- Day timeline: snapshots each day on New Day, reverse-chronological cards, markdown export.
...
|`js/dmscreen/partytracker/dmscreen-partytracker-import.js`|`PartyTrackerImporter` (character sheet JSON → Party Tracker data)|
|`js/dmscreen/dmscreen-journeytracker.js`|`JourneyTracker` + `JourneyTrackerRoot` (full journey/camp/weather/supply/timeline implementation)|
...
**Effect verification is a first-class requirement.**  Every
`featuresMatrix` entry that points at a feature with a measurable
derived effect (passive bonus, advantage flag, toggle delta, attack
rider, resource cost, …) must attach at least one `EffectCheck` —
existence-only assertions are insufficient (standard check #22).  Use
the per-pick `pickedFeatureGrants` mechanism (Phase 11) and the
`build*Checks` helpers in
[`test/e2e/utils/tgttFeaturePools.ts`](test/e2e/utils/tgttFeaturePools.ts)
(auto-generated by `node scripts/genTgttPools.mjs`) to verify that
specialty / Battle Tactic / Metamagic / Invocation / Jester Act /
Trickster Trick / Precise Strike / Pact Boon / Dreamwalker call /
...
|When to read|File|
|---|---|
|The 22 required checks every comprehensive spec must cover (or skip with reason); effect-verification doctrine|[standard.md](.agents/skills/e2e-character-tests/references/standard.md)|
|Copy-paste template + 10-step authoring checklist + per-pick effect probe checklist|[spec-template.md](.agents/skills/e2e-character-tests/references/spec-template.md)|
...
|How CharacterSpec and MulticlassCharacterSpec become individual tests; effect-probe dispatch|[factory-tests.md](.agents/skills/e2e-character-tests/references/factory-tests.md)|
...
|Catalog of every TGTT character spec with build, level scope, "what it proves"|[docs/e2e/test-suite-catalog.md](docs/e2e/test-suite-catalog.md)|
