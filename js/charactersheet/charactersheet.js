...
class CharacterSheetPage {
	...
	static _buildCarryTooltip (b) {
		...
		// Body capacity line (Strength-based). When extradimensional storage is in
		// play we show the body subtotal here and the combined total on its own line,
		// so the two contributions stay visually distinct.
		const bodyLine = factors.length
			? `${label}: (${head} = ${b.base} lb.) ${factors.join(" ")} = ${b.bodyCapacity} lb.`
			: `${label}: ${head} = ${b.bodyCapacity} lb.`;
	...
	_renderOverviewSpecialtiesFeats () {
		...
		// (R47 Bug 2) Structured-choice sub-options carry `isFeatureOption:true`. Divine Order is a
		// ONE-TIME level-1 build choice (role: Protector / Thaumaturge), so it is shown HERE as its
		// own READ-ONLY category rather than a mid-play-changeable field. Principles of Devotion, by
		// contrast, CAN change mid-play and keeps its own dedicated Overview surface, so its picks
		// must NOT be duplicated in this list. Genuine TGTT Specialties (and Blessed Strikes, etc.)
		// are feature-options with OTHER parents and still appear under "Specialties". Parentage is
		// read from BOTH a feature's own `parentFeature` field AND the durable `chosenSubfeatures`
		// linkage (name+source→parent), so a save that dropped `parentFeature` is still classified.
		...
		const featureOptions = (this._state.getFeatures?.() || []).filter(f => f.isFeatureOption === true);
		const divineOrder = featureOptions.filter(f => isDivineOrderOption(f));
		const specialties = featureOptions.filter(f => !isExcluded(f) && !isDivineOrderOption(f));

		if (!feats.length && !specialties.length && !divineOrder.length) {
		...
		buildGroup("Divine Order", "⚜️", divineOrder);
		...
		if (availableFeatures.length > 0 || !hasActiveStates) {
			...
			if (availableFeatures.length === 0) {
				...
				availableFeatures.forEach(({feature, activationInfo, resource, stateTypeId}) => {
					...
					// NOTE: two mechanisms for "restore a use by expending a spell
					// slot" coexist here — the generic `restoreWithSpellSlotLevel`
					// (drives the separate restore button below) and Daemonologist's
					// bespoke `canRestoreUnearthlyCountenanceUse` (keeps the Use
					// button enabled). Both are preserved so neither subclass
					// regresses; converging Unearthly Countenance onto the generic
	...
	_renderAttacks () {
		...
		displayAttacks.forEach(attack => {
			...
			const rangeProjection = this._state.getAttackRangeProjection?.(attack, {meleeReach: overviewReachCtx.meleeReach});
			const rangeStr = rangeProjection?.display || attack.range || (attack.isMelee ? "5 ft." : "");
	...
	async _rollAttack (attack, event) {
		...
		// Only add the opted-in conditional numeric bonus on top of the canonical
		// standing total. This keeps roll-only choices outside the row projection.
		const conditionalAttackBonus = appliedConditionals.reduce((acc, c) => acc + (c.bonus || 0), 0);
		const attackBreakdown = this._state.getAttackBonusBreakdown?.(attack);
		const standingAttackBonus = attackBreakdown?.total ?? 0;
		const attackTotal = rollResult.roll + standingAttackBonus + conditionalAttackBonus - exhaustionPenalty;
