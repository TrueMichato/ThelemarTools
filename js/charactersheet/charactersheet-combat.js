...
class CharacterSheetCombat {
	...
	_resolveAttackNoteStorage (attackOrId) {
		const attack = typeof attackOrId === "object"
			? attackOrId
			: this._findAttackById(attackOrId);
		if (!attack?.id) return null;

		const stateAttack = (this._state.getAttacks?.() || []).find(it => it.id === attack.id);
		if (stateAttack) return {attack: stateAttack, storage: "stateAttack"};

		const isGeneratedWeapon = attack.isAutoGenerated || attack.id.startsWith?.("auto_");
		if (!isGeneratedWeapon) return {attack, storage: null};

		const itemId = attack.sourceItem?.id
			|| (attack.id.startsWith?.("auto_") ? attack.id.slice(5) : null);
		if (!itemId) return {attack, storage: null};
		const item = this._state.getItemRaw?.(itemId);
		return item ? {attack, storage: "itemOverride", itemId} : {attack, storage: null};
	}

	_getAttackNote (attackOrId) {
		const resolved = this._resolveAttackNoteStorage(attackOrId);
		if (!resolved?.storage) return "";
		if (resolved.storage === "stateAttack") return this._state.getAttackNote?.(resolved.attack.id) || "";
		return this._state.getItemRaw?.(resolved.itemId)?.attackOverrides?.note || "";
	}
	...
	_saveAttackNote (attackId, note) {
		...
		let isUpdated = false;
		if (resolved.storage === "stateAttack") {
			isUpdated = this._state.updateAttackNote?.(resolved.attack.id, note) || false;
		} else {
			const current = this._state.getItemRaw?.(resolved.itemId);
			if (!current) return false;
			const nextOverrides = {...(current.attackOverrides || {})};
			if (note) nextOverrides.note = note;
			else delete nextOverrides.note;
			isUpdated = this._state.updateInventoryItemAttackOverrides?.(resolved.itemId, {
