...
class CharacterSheetCombat {
	...
	_initEventListeners () {
		...
		document.addEventListener("click", (/** @type {*} */ e) => {
			...
			const attackId = target.closest(".charsheet__attack-item")?.dataset.attackId;
			if (!attackId) return;
			void this._showAttackNoteModal(attackId);
		});
	...
	async _pShowAttackModal (existingAttack = null) {
		...
		const bonus = eff ? (eff.totalAttackBonus || 0) : (this._parseBonus(weapon.bonusWeapon) + this._parseBonus(weapon.bonusWeaponAttack));
		...
		if (inventorySelect) {
			inventorySelect.addEventListener("change", () => {
				...
				if (weapon) {
					...
					let damageBonusVal;
					if (eff) {
						attackBonusVal = eff.totalAttackBonus || 0;
						damageBonusVal = eff.totalDamageBonus || 0;
					} else {
						attackBonusVal = this._parseBonus(weapon.bonusWeapon) + this._parseBonus(weapon.bonusWeaponAttack);
						damageBonusVal = this._parseBonus(weapon.bonusWeapon) + this._parseBonus(weapon.bonusWeaponDamage);
					}
	...
	async _pShowWeaponAttackModal (weapon) {
		...
		// Derivation, not the raw entry — see `_pShowWeaponBonusModal`.
		const effBonuses = this._state.getEffectiveItemBonuses?.(weapon.id);
		const magicBonus = effBonuses
			? (effBonuses.totalAttackBonus || 0)
			: this._parseBonus(weapon.bonusWeapon) + this._parseBonus(weapon.bonusWeaponAttack);
		const magicDmgBonus = effBonuses
			? (effBonuses.totalDamageBonus || 0)
			: this._parseBonus(weapon.bonusWeapon) + this._parseBonus(weapon.bonusWeaponDamage);
		...
		footer.querySelector(".charsheet__attack-btn--reset").addEventListener("click", () => {
			...
			this._persistWeaponAttackOverrides(weapon.id, {
	...
	async _pShowWeaponBonusModal (weapon) {
		...
		// Show the weapon's base stats and allow editing bonuses. Read the DERIVATION rather than
		// the raw entry, so applied upgrades are counted here too — reading `weapon.*` directly
		// told the player "none" for a weapon carrying three upgrades.
		const effBonuses = this._state.getEffectiveItemBonuses?.(weapon.id);
		const magicBonus = effBonuses
			? (effBonuses.totalAttackBonus || 0)
			: this._parseBonus(weapon.bonusWeapon) + this._parseBonus(weapon.bonusWeaponAttack);
	...
	/**
	 * Resolve where an attack note can be stored. Manual attacks own their note
	 * directly; generated weapon attacks persist it with the backing item's attack
	 * overrides. Transient attacks have no durable storage and therefore no note action.
	 * @param {string|object} attackOrId
	 * @returns {{attack: object, storage: "stateAttack"|"itemOverride"|null, itemId?: string}|null}
	 */
	_resolveAttackNoteStorage (attackOrId) {
		return CharacterSheetCombat.resolveAttackNoteStorage(this._state, attackOrId, {
			findAttackById: attackId => this._findAttackById(attackId),
		});
	}

	static resolveAttackNoteStorage (state, attackOrId, {findAttackById = null} = {}) {
		const attackId = typeof attackOrId === "string" ? attackOrId : attackOrId?.id;
		if (!attackId) return null;

		const stateAttack = (state.getAttacks?.() || []).find(it => it.id === attackId);
		if (stateAttack) return {attack: stateAttack, storage: "stateAttack"};

		const attack = typeof attackOrId === "object"
			? attackOrId
			: findAttackById?.(attackId);
		const isGeneratedWeapon = attack?.isAutoGenerated || attackId.startsWith("auto_");
		if (!isGeneratedWeapon) return {attack, storage: null};

		const itemId = attack?.sourceItem?.id
			|| (attackId.startsWith("auto_") ? attackId.slice(5) : null);
		if (!itemId) return {attack, storage: null};
		const item = state.getItemRaw?.(itemId);
		return item ? {attack: attack || {id: attackId, name: item.name || "Attack"}, storage: "itemOverride", itemId} : {attack, storage: null};
	}

	_getAttackNote (attackOrId) {
		return CharacterSheetCombat.getAttackNote(this._state, attackOrId, {
			findAttackById: attackId => this._findAttackById(attackId),
		});
	}

	static getAttackNote (state, attackOrId, opts) {
		const resolved = CharacterSheetCombat.resolveAttackNoteStorage(state, attackOrId, opts);
		if (!resolved?.storage) return "";
		if (resolved.storage === "stateAttack") return state.getAttackNote?.(resolved.attack.id) || "";
		return state.getItemRaw?.(resolved.itemId)?.attackOverrides?.note || "";
	}

	/**
	 * Persist weapon attack overrides without allowing a stat edit/reset to erase
	 * the generated attack's note.
	 */
	_persistWeaponAttackOverrides (itemId, {attackOverrides, customAttackBonus, customDamageBonus} = {}) {
		const current = this._state.getItemRaw?.(itemId);
		if (!current) return false;

		const currentNote = current.attackOverrides?.note;
		let nextOverrides = attackOverrides;
		if (attackOverrides === null) {
			nextOverrides = currentNote === undefined ? null : {note: currentNote};
		} else if (attackOverrides !== undefined && currentNote !== undefined && attackOverrides.note === undefined) {
			nextOverrides = {...attackOverrides, note: currentNote};
		}

		return this._state.updateInventoryItemAttackOverrides?.(itemId, {
			attackOverrides: nextOverrides,
			customAttackBonus,
			customDamageBonus,
		}) || false;
	}

	_saveAttackNote (attackId, note) {
		const isUpdated = CharacterSheetCombat.updateAttackNote(this._state, attackId, note, {
			findAttackById: attackIdInner => this._findAttackById(attackIdInner),
		});
	...
	static updateAttackNote (state, attackOrId, note, opts) {
		const resolved = CharacterSheetCombat.resolveAttackNoteStorage(state, attackOrId, opts);
		if (!resolved?.storage) return false;

		const cleanNote = note?.trim() || "";
		if (resolved.storage === "stateAttack") return state.updateAttackNote?.(resolved.attack.id, cleanNote) || false;

		const current = state.getItemRaw?.(resolved.itemId);
		if (!current) return false;
		const nextOverrides = {...(current.attackOverrides || {})};
		if (cleanNote) nextOverrides.note = cleanNote;
		else delete nextOverrides.note;
		return state.updateInventoryItemAttackOverrides?.(resolved.itemId, {
			attackOverrides: Object.keys(nextOverrides).length ? nextOverrides : null,
		}) || false;
	}

	async _showAttackNoteModal (attackId) {
		const resolved = this._resolveAttackNoteStorage(attackId);
		if (!resolved?.storage) return;

		const currentNote = this._getAttackNote(resolved.attack);
		const {eleModalInner: modalInner, doClose} = await CharacterSheetModal.pGetShow({
			title: `📝 Note: ${resolved.attack.name}`,
			isMinHeight0: true,
		});

		const textarea = e_({tag: "textarea", clazz: "ve-form-control"});
		textarea.rows = 6;
		textarea.placeholder = "Add notes about this attack...";
		...
		if (currentNote) {
			e_({
				...
				txt: "Clear Note",
				click: () => {
					if (!this._saveAttackNote(attackId, "")) {
						JqueryUtil.doToast({type: "warning", content: "Could not clear the note for this attack."});
						return;
					}
		...
		e_({
			...
			txt: "Save Note",
			click: () => {
				if (!this._saveAttackNote(attackId, textarea.value)) {
					JqueryUtil.doToast({type: "warning", content: "Could not save the note for this attack."});
					return;
				}
	...
	_renderAttackItem (attack, reachCtx = {}) {
		...
		// Hoverable name. Routed through the SHARED builder, which decides for itself whether an
		// item still matches its catalog entity: a custom weapon has no catalog entry to link to,
		// and a materialled or upgraded one would resolve to the pristine version and quietly
		// misreport its own damage. Hard-coding `{@item …}` here opted this surface out of both.
		// Only weapon-backed rows go through it — a feature or spell attack is not an item and
		// must not be resolved against the item catalog.
		const nameHtml = attack.sourceItem?.name
			? CharacterSheetClassUtils.buildItemHoverNameHtml(attack.sourceItem, {displayLabel: attack.name})
			: CharacterSheetClassUtils.escapeHtml(attack.name);
		...
		return e_({outer: `
			<div class="charsheet__attack-item" data-attack-id="${attack.id}">
				...
				<div class="charsheet__attack-actions">
					...
					${attackNoteStorage?.storage ? `<button class="ve-btn ve-btn-sm ${attackNote ? "ve-btn-warning" : "ve-btn-default"} charsheet__attack-note" title="${attackNote ? "Edit Note" : "Add Note"}">
	...
	static get ACTION_ECONOMY_FEATURE_PROMOTIONS () {
		return [{
			featureName: "cunning action",
			targetType: "bonus",
			actionNames: ["Dash", "Disengage", "Hide"],
		}];
	}
	...
	_applyActionEconomyFeaturePromotions (buckets, standardEntries) {
		for (const promotion of CharacterSheetCombat.ACTION_ECONOMY_FEATURE_PROMOTIONS) {
			const parent = buckets[promotion.targetType]
				.find(entry => entry.kind === "feature" && entry.name?.toLowerCase() === promotion.featureName);
			if (!parent || parent.children?.length) continue;

			const children = promotion.actionNames
				.map(actionName => standardEntries.find(entry => entry.name === actionName && entry.actionType === "action"))
				.filter(Boolean)
				.map(entry => ({
					...entry,
					id: `${parent.id}:promoted:${entry.id}`,
					actionType: promotion.targetType,
					kindLabel: `Bonus Action granted by ${parent.name}`,
					subtitle: `Bonus Action · via ${parent.name}`,
					parentId: parent.id,
					relation: "promoted-action",
					promotedFrom: entry.actionType,
				}));
	...
	getCombatActionEconomy () {
		...
		const items = this._state.getItems?.() || [];
		for (const weapon of items.filter(i => i.weapon
			&& i.equipped
			&& (this._state.isItemAttackAvailable?.(i) ?? true))) {
		...
		this._applyActionEconomyFeaturePromotions(buckets, standardEntries);
	...
	_featureEconomySubtitle (feature) {
		...
		if (globalThis.CharacterSheetClassUtils?.isCombatMethod?.(feature)) {
			return feature.staminaCost ? `Combat Method · ${feature.staminaCost} stamina` : "Combat Method";
		}
	...
	renderCombatActionEconomy () {
		...
		const buildRow = (entry) => {
			const baseMeta = kindMeta[entry.kind] || {label: "", glyph: "•"};
			const meta = entry.kindLabel ? {...baseMeta, label: entry.kindLabel} : baseMeta;
		...
		const buildEntry = (entry) => {
			...
			const family = e_({tag: "div", clazz: "cs-combat-action-economy__family"});
			family.setAttribute("role", "group");
			family.setAttribute("aria-label", `${entry.name} — Bonus Actions it grants`);
			family.appendChild(row);

			const children = e_({tag: "div", clazz: "cs-combat-action-economy__children"});
			children.setAttribute("role", "list");
			for (const child of entry.children) {
				const childRow = buildRow(child);
				childRow.setAttribute("role", "listitem");
				childRow.setAttribute("aria-label", `${child.name}, Bonus Action granted by ${entry.name}`);
		...
		for (const col of columns) {
			...
			const visibleCount = entries.reduce((count, entry) => count + 1 + (entry.children?.length || 0), 0);
			const group = e_({tag: "div", clazz: "cs-combat-action-economy__group"});

			const header = e_({outer: `<div class="cs-combat-action-economy__head">${csCombatActionChip(col.chip)}<span class="cs-combat-action-economy__count">${visibleCount}</span></div>`});
			...
			if (!entries.length) {
				...
				for (const entry of entries.filter(e => !collapsedSet.has(e.kind))) list.appendChild(buildEntry(entry));
