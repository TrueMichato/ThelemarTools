	...
	renderAttacks () {
		...
		equippedWeapons.forEach(weapon => {
			...
			if (!existingAttack) {
				...
				// #14: standing weapon-upgrade damage-dice rider (e.g. Saw-toothed = +1d4
				// slashing on every hit). S6 surfaces the aggregated extra die + type via
				// getEffectiveItemBonuses.bonusDamageDice/bonusDamageType; we attach it to the
				// attack so `_rollDamage` rolls it (crit-doubled, under its OWN type — falling
				// back to the weapon's type only if the upgrade left the type unspecified) on
				// every hit. Read defensively so this is inert until S6 ships the surface.
				const weaponDamageRiders = [];
				if (effectiveBonuses?.bonusDamageDice) {
					weaponDamageRiders.push({
						source: "Weapon Upgrade",
						dice: effectiveBonuses.bonusDamageDice,
						damageType: effectiveBonuses.bonusDamageType || baseDamageType,
					});
				}
				...
				const autoAttack = {
					...
					weaponDamageRiders, // #14: standing weapon-upgrade damage-dice riders
				};
