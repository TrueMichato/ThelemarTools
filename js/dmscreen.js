...
import {
	...
	PanelContentManager_NoteBox,
	PanelContentManager_PartyTracker,
	PanelContentManager_TimeTracker,
...
class AddMenuSpecialTab extends AddMenuTab {
	...
	async pRender () {
		if (!this.eleTab) {
			...
			const btnPartyTracker = veT`<button class="ve-btn ve-btn-primary ve-btn-sm">Add</button>`
				.vee.onn("click", async () => {
			...
			const btnNpcTracker = veT`<button class="ve-btn ve-btn-primary ve-btn-sm">Add</button>`
				.vee.onn("click", async () => {
					const pcm = new PanelContentManager_NpcTracker({board: this._board, panel: this.menu.pnl});
					await pcm.pDoPopulate();
					this.menu.doClose();
				});
			...
			const wrpDiceCalculator = veT`<div class="ve-ui-modal__row"><span>Dice Calculator</span></div>`.vee.appendTo(eleTab);
			const btnDiceCalculator = veT`<button class="ve-btn ve-btn-primary ve-btn-sm">Add</button>`.vee.appendTo(wrpDiceCalculator);
			btnDiceCalculator.vee.onn("click", async () => {
				const pcm = new PanelContentManager_DiceCalculator({board: this._board, panel: this.menu.pnl});
				await pcm.pDoPopulate();
				this.menu.doClose();
			});

			const wrpRandomTable = veT`<div class="ve-ui-modal__row"><span class="ve-help" title="A user-editable random table — add rows, then click Roll to pick one. Row text supports content tags like {@spell fireball} and {@creature goblin}.">Random Table</span></div>`.vee.appendTo(eleTab);
			const btnRandomTable = veT`<button class="ve-btn ve-btn-primary ve-btn-sm">Add</button>`.vee.appendTo(wrpRandomTable);
			btnRandomTable.vee.onn("click", async () => {
			...
			const wrpJourneyTracker = veT`<div class="ve-ui-modal__row"><span>Journey & Camp Tracker</span></div>`.vee.appendTo(eleTab);
			const btnJourneyTracker = veT`<button class="ve-btn ve-btn-primary ve-btn-sm">Add</button>`.vee.appendTo(wrpJourneyTracker);
			btnJourneyTracker.vee.onn("click", async () => {
				const pcm = new PanelContentManager_JourneyTracker({board: this._board, panel: this.menu.pnl});
				await pcm.pDoPopulate();
				this.menu.doClose();
			});

			const wrpItemBuilder = veT`<div class="ve-ui-modal__row"><span class="ve-help" title="Build a canonical homebrew item from a catalog preset, material, upgrades, and gemstone empowerment.">Item Builder</span></div>`.vee.appendTo(eleTab);
			const btnItemBuilder = veT`<button class="ve-btn ve-btn-primary ve-btn-sm">Add</button>`.vee.appendTo(wrpItemBuilder);
			btnItemBuilder.vee.onn("click", async () => {
				const pcm = new PanelContentManager_ItemBuilder({board: this._board, panel: this.menu.pnl});
				await pcm.pDoPopulate();
				this.menu.doClose();
			});
