...
- Transfer acceptance refreshes canonical character documents, shared inventory, balances, source/target/item
  ...
  cannot discover the recovery, prove no `clientImportId` rematch or CREATE occurs, and verify a later save uses
  a fresh character identity. A real-browser dropdown regression drives the production
