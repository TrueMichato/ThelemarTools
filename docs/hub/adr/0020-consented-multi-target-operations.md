...
> future additive migration `0011_multi_target_semantic_operations.sql` in A3, after A2's reserved immutable
> `0010_source_cost_binding_identity.sql`. The draft must remain unmerged until the coordinator records the physical game-day GO/NO-GO.
...
Migration `0011_multi_target_semantic_operations.sql` is required in A3. A2 owns the immediately preceding
immutable migration `0010_source_cost_binding_identity.sql` for shared/PostgreSQL resource-binding identity and
ABA correctness. The normalized target table is
...
rollback target must be an A3-aware bridge release which understands `target_set_version`, child history,
projection filtering, expiry, retention, explicit cleanup, and the usage marker. A pre-0011 rollback preflight requires the
...
| **A3 — server state machine** | Migration 0011 after A2's migration 0010, protocol 6, capability/routes, both stores, events, lifecycle, privacy, expiry, locks | Seed-10 cross-campaign quota races, opposing-order deadlock tests, usage-marker persistence, fault injection, purge/bridge rollback |
