...
| `CHARACTER_RECOVERY_EXACT_REQUEST_UNAVAILABLE` | recovery format, whether the original PATCH body and rules-version pin were stored, whether the command contains one-shot activity | legacy browser recovery created before format 3 | export the local character before discarding recovery or choosing server state; never reconstruct the activity request under its old key or rotate the key and risk duplicating the event |
