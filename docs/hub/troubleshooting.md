...
| Item award is missing, duplicated, has type `Other`, lost metadata, or has the wrong stash count | request/idempotency key, `item.award_batch` audit, ordered `item.granted` events, authoritative character/stash revision, exact `name\|source`; remember the event item is intentionally summary-only | trusted award resolver/catalog freshness, award receipt, target locks, stash transaction, Character Sheet catalog rehydration, outbox/reconciliation | stop further inventory mutations for the campaign; compare only safe aggregate evidence until authorized to inspect the canonical document, and follow the incident runbook rather than compensating manually |
