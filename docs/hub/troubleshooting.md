...
| `CHARACTER_RECOVERY_LIMIT` / `CHARACTER_RECOVERY_STORAGE_UNAVAILABLE` | pending command count, recovery payload bytes, browser `sessionStorage` availability/quota, temporary-to-canonical identity migration | ordered character recovery queue | wait for the current save, retry, or export; never bypass recovery durability, discard the pending activity, or remove the temporary recovery key unless the canonical replacement was stored |
