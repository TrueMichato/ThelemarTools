...
| Item award is missing, duplicated, has type `Other`, lost metadata, or has the wrong stash count | request/idempotency key, `item.award_batch` audit, ordered `item.granted` events, authoritative character/stash revision, exact `name\|source`; remember the event item is intentionally summary-only; for duplicate stacks compare deterministic Character Sheet aliases separately from custom/composition data | trusted award resolver/catalog freshness, canonical stack equivalence after sheet save, award receipt, target locks, stash transaction, Character Sheet repair-projection capture order and cache filtering, outbox/reconciliation | stop further inventory mutations for the campaign; compare only safe aggregate evidence until authorized to inspect the canonical document, and follow the incident runbook rather than compensating manually |
