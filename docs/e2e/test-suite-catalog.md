...
| 26 | `tgtt-lunar-sorcery-sorcerer.spec.ts` | Lunar Sorcery Sorcerer (DSotDQ) · Human | 1–20 | The moon phase as a real, switchable, mutually-exclusive state seeded from the Builder/Level-Up/Quick Build choice and gating four separate mechanics: which 1st-level Lunar spell is free, which two schools Lunar Boons discounts off `getMetamagicCost()`, which Lunar Empowerment passive is live (Crescent's `damage:necrotic`/`damage:radiant` resistances asserted with negative controls in the other two phases), and which Lunar Phenomenon payload fires. Also the fifteen-spell Lunar Spells table granted through the new declarative `FEATURE_SPELL_GRANTS` registry (the shipped `additionalSpells` block transcribes only one of three columns), Moon Fire's off-list Sacred Flame cantrip, and Waxing and Waning's exactly-1-sorcery-point bonus-action switch. |
