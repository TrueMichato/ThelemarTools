...
**Activated speed items:** DMG and XDMG Boots of Speed grant `modifySpeed.multiply` only while equipped, attuned, and deliberately activated through their bonus-action item power. Once active, the exact owned item's name and power appear as status-only rows in Overview, Combat, and Play Mode's Active States displays. **Manage power** opens and focuses that item's existing Powers modal; it is not another activation switch. The rows are derived from the item's saved `itemPowerStates[power.id].active` flag, never copied into the character's `activeStates` records. They remain visible when activation has spent the last resource, across tab changes, rests, and save/reload, and disappear on actual power deactivation or on unequip/unattune. An already-active operational toggle can be deactivated even after its last charge, use, or resource is spent; turning it off costs nothing, while turning it back on still requires the activation cost. The Overview speed and its breakdown refresh when the power is toggled; unequipping or ending attunement turns the speed power off, so re-equipping requires another activation. A save with unrelated item powers but no speed toggle receives the catalog toggle on load, and the old XDMG reference-only card is replaced rather than duplicated. An authored custom item with `modifySpeed.multiply` but no speed power is intentionally passive while equipped/attuned and never appears as an activated status.
