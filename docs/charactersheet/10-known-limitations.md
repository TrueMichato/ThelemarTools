...
| Source-data typos pass through | e.g. TGTT's "methoding" in Blinding Blow Stance is a botched maneuver→method rename in the data, not an exporter defect |
| Real-save regression fixtures untracked | `npc-exports/*.json` are personal character data; `CharacterSheetNpcExporter.realsaves.test.js` skips when absent, so CI covers less than local. The local corpus is twenty-one characters (Aldor, Arthur, Boti, Dauk, Dranan, Duralin, Dzeiy, Fili, Juen, Lorian, Mikase, Nessa, Octavius, Onger, Phirse, Reggu, Talna, Tignor, Tikal, Vern, Wisp) |
