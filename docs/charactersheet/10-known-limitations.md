...
| A power's `N/Day` band is a model, not a rule | The strain→uses conversion matches the book's own roster assignments on 140 of 181 entries (77%; order-1 → at-will is 28/28). The residual is authorial: the same power at the same order is filed `3/Day` in one published statblock and `1/Day` in another, so no order-based model can do better |
| Upcast compression covers damage, targets and area only | `_compressPsionicUpcast` resolves "the damage increases by NdX", "targets one additional creature" and "the radius/length/range increases by N feet". Any other increased-order shape keeps its original prose rather than being guessed at |
| Only one corpus character manifests | Phirse is the entire runtime test surface for psionics, so the 27 published statblocks serve as the second opinion. v18's contract tests are deliberately structural rather than keyed to power names, because the sheet's power resolution is itself under active development |
...
## NPC export — the action-economy mark needs a hover to be complete

The exporter writes a roster line's action economy as a superscript that names
itself on hover: `{@sup {@tip B|Bonus Action}}` renders a raised **B** whose
`title` reads *Bonus Action* (see
[18-npc-export.md](./18-npc-export.md), v19). Three surfaces do not carry the
hover, so on those the mark is only as good as the glyph:

| surface | ᴬ / ᴮ / ᴿ | ¹⁰ᵐⁱⁿ / ¹ʰʳ |
|---|---|---|
| rendered HTML | full — glyph plus hover | full |
| **touch device** | glyph only; `title=` never fires | **complete** — the time is printed |
| **markdown export** | recovered — prints `(Bonus Action)` | recovered |
| **plain-text / `stripTags`** | glyph only | **complete** |

This is the reason long casting times print the time rather than an opaque
"else" letter: `Ceremony ¹ʰʳ` is readable with no hover at all, whereas an `E`
would have been meaningless on a tablet. The three single letters are accepted
as learnable in context — a roster of spells with a raised `B` on two of them
reads as an economy hint even to someone who has never hovered one.

A statblock must therefore never state a mechanically load-bearing fact *only*
in a mark. Today the mark is always redundant with the spell or power itself,
so nothing is lost when the hover is unavailable.

**Unmarked means unreadable, not "action".** A roster line with no mark is a
line whose casting time the exporter could not determine — currently two of 487
corpus spell rows, plus always-prepared entries built from bare `name|source`
references. It is deliberately not a silent default.
