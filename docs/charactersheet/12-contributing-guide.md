...
active when a full modal is explicitly opened; the result is embedded there rather than exposed by
weakening modal safety.

For optional **post-attack** decisions, do not open that modal automatically. Add a labelled
`deferred` entry to Combat's `_getPostAttackHooks()`: the common side offer keeps the real
